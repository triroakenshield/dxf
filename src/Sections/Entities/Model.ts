...
export default class Model extends DXFManager {
    ...
    get attributes(): AttributeDefinition[] { return this._attributes; }
    ...
    public addInsert(blockName: string, x: number, y: number, z: number): BlockReference {
        var res = new BlockReference(blockName, new Point(x, y, z));
        this._blockReferences.push(res);
        return res;
    }

    public addAttribute(attributeName: string, x: number, y: number, z: number) {
        this._attributes.push(new AttributeDefinition(attributeName, new Point(x, y, z)));
    }
